...
bool clipboardCopyManager::keyQuickAltPasteCheck()
{
	if (GetAsyncKeyState(scanCode_specialF4) && (GetAsyncKeyState(VK_LCONTROL) || GetAsyncKeyState(VK_RCONTROL)))
	{
		return true;
	}
...
bool clipboardCopyManager::keyQuickPasteCheck()
{
	if (GetAsyncKeyState(scanCode_specialF4))
	{
		return true;
	}
...
bool clipboardCopyManager::keyQuickPasteCheck(int toCheck)
{
	if (GetAsyncKeyState(toCheck))
	{
		return true;
	}
...
void clipboardCopyManager::spoolUp()
{
	...
	while (true)
	{
		...
		case pasteRequestedPattern3KeyDown:
			...
			break;
		case pasteRequestedQuickKeyDown:
			if (checkForKeyup())
			{
				currentState = stateMachineState::waitingForInput;
				std::string toStore = GetClipboardText();
				...
				{
					scanCodedMessage tempMessage = translateToScanCodedMessage(toStore);
					typeScanCodedMessagePattern3(tempMessage);
				}
			}
			break;
		case pasteRequestedAltQuickKeyDown:
			if (checkForKeyup())
			{
				Sleep(1500);
				currentState = stateMachineState::waitingForInput;
			...
			break;
		case pasteRequestedBackupSuccessfulKeyDown:
			if (checkForKeyup())
			{
				currentState = stateMachineState::waitingForInput;
				std::string toEncode = "Backup successful.\n";
				scanCodedMessage tempMessage = translateToScanCodedMessage(toEncode);
				typeScanCodedMessagePattern3(tempMessage);
			}
			break;
		case pasteRequestedBackupDayBehindKeyDown:
			if (checkForKeyup())
			{
				currentState = stateMachineState::waitingForInput;
				std::string toEncode = "Backups a day behind\n";
				scanCodedMessage tempMessage = translateToScanCodedMessage(toEncode);
				typeScanCodedMessagePattern3(tempMessage);
			}
			break;
		case pasteRequestedStartedOfflineBackupKeyDown:
			if (checkForKeyup())
			{
				currentState = stateMachineState::waitingForInput;
				std::string toEncode = "Offline.\n";
				scanCodedMessage tempMessage = translateToScanCodedMessage(toEncode);
				typeScanCodedMessagePattern3(tempMessage);
			}
			break;
		case pasteRequestedStartedNASBackupKeyDown:
			if (checkForKeyup())
			{
				currentState = stateMachineState::waitingForInput;
				std::string toEncode = "Started NAS backup.\n";
				scanCodedMessage tempMessage = translateToScanCodedMessage(toEncode);
				typeScanCodedMessagePattern3(tempMessage);
			}
			break;
		case pasteRequestedStartedCloudBackupKeyDown:
			if (checkForKeyup())
			{
				currentState = stateMachineState::waitingForInput;
				std::string toEncode = "Started cloud backup.\n";
				scanCodedMessage tempMessage = translateToScanCodedMessage(toEncode);
				typeScanCodedMessagePattern3(tempMessage);
			}
			break;
		case waitingForInput:
			...
			if (keyQuickAltPasteCheck())
			{
				currentState = pasteRequestedAltQuickKeyDown;
			}
			else if (keyQuickPasteCheck())
			{
				currentState = pasteRequestedQuickKeyDown;
			}
			else if (keyQuickPasteCheck(scanCode_specialF3))
			{
				currentState = pasteRequestedBackupSuccessfulKeyDown;
			}
			else if (keyQuickPasteCheck(scanCode_specialF2))
			{
				currentState = pasteRequestedBackupDayBehindKeyDown;
			}
			else if (keyQuickPasteCheck(scanCode_specialF5))
			{
				currentState = pasteRequestedStartedOfflineBackupKeyDown;
			}
			else if (keyQuickPasteCheck(scanCode_specialF6))
			{
				currentState = pasteRequestedStartedNASBackupKeyDown;
			}
			else if (keyQuickPasteCheck(scanCode_specialF7))
			{
				currentState = pasteRequestedStartedCloudBackupKeyDown;
			}
			else if (keyCombinationCopyCheck())
			{
